...
# 2. ESTILOS CSS (Centrado Forzado y Diseño de Burbuja)
st.markdown("""
    <style>
    ...
    .question-text { text-align: center; font-size: 26px !important; font-weight: 700; color: #1E3A8A; margin: 30px auto; max-width: 800px; }
    .info-text { text-align: center; font-size: 14px; color: #718096; margin-top: -10px; margin-bottom: 20px; font-style: italic; }
    ...
    .ideology-title { font-size: 42px !important; font-weight: 900; color: #2B6CB0; text-transform: uppercase; }
    ...
    /* Separadores */
    .separator { border-top: 2px solid #CBD5E0; margin: 25px auto; width: 550px; }
    .bubble-sep { border-top: 2px solid #CBD5E0; margin: 15px auto; width: 60%; }

    /* Botón volver (más fino) */
...
# 3. DATOS
LEADERS = [
    {"n": "Stalin", "x": -9, "y": 8.5, "c": "red"},
    {"n": "Mao", "x": -9.2, "y": 7.5, "c": "red"},
    {"n": "Castro", "x": -8, "y": 6, "c": "green"},
    {"n": "Hitler", "x": 8, "y": 9, "c": "#333"},
    {"n": "Thatcher", "x": 7, "y": 6, "c": "blue"},
    {"n": "Gandhi", "x": -6, "y": -7, "c": "#68D391"},
    {"n": "Sanders", "x": -4, "y": -3, "c": "#63B3ED"},
    {"n": "Friedman", "x": 7.5, "y": -5.5, "c": "orange"},
...
if st.session_state.idx >= len(questions):
    ...
    compass_html = f"""
    <style>
        .map {{ position: relative; width: 450px; height: 450px; margin: auto; background: white; border: 2px solid #000; font-family: sans-serif; overflow: visible; }}
        .quadrant {{ position: absolute; width: 50%; height: 50%; opacity: 0.15; }}
        .axis-h {{ position: absolute; width: 100%; height: 2px; background: #000; top: 50%; z-index: 2; }}
        .axis-v {{ position: absolute; width: 2px; height: 100%; background: #000; left: 50%; z-index: 2; }}
        .q-label {{ position: absolute; font-size: 11px; font-weight: bold; color: #000; z-index: 3; }}
        .dot {{ position: absolute; width: 10px; height: 10px; border-radius: 50%; transform: translate(-50%, -50%); border: 1px solid #000; z-index: 4; }}
        .label {{ position: absolute; font-size: 10px; font-weight: bold; transform: translate(-50%, 6px); width: 60px; text-align: center; z-index: 4; }}
        .user-dot {{ position: absolute; width: 20px; height: 20px; background: red; border-radius: 50%; transform: translate(-50%, -50%); border: 2px solid white; box-shadow: 0 0 8px red; z-index: 10; }}
        .user-txt {{ position: absolute; font-size: 14px; font-weight: 900; color: red; transform: translate(-50%, 12px); z-index: 11; }}
    </style>
    <div class="map">
        <div class="quadrant" style="top:0; left:0; background: #f9b4b4;"></div>
        <div class="quadrant" style="top:0; right:0; background: #b4c7f9;"></div>
        <div class="quadrant" style="bottom:0; left:0; background: #b4f9c7;"></div>
        <div class="quadrant" style="bottom:0; right:0; background: #f9f9b4;"></div>
        <div class="axis-h"></div><div class="axis-v"></div>
        <div class="q-label" style="top:-20px; left:41%;">AUTORITARIO</div>
        <div class="q-label" style="bottom:-20px; left:42%;">LIBERTARIO</div>
        <div class="q-label" style="top:48%; left:-65px;">IZQUIERDA</div>
        <div class="q-label" style="top:48%; right:-60px;">DERECHA</div>
        {leaders_js}
        <div class="user-dot" style="left:{user_x}%; top:{user_y}%;"></div>
        <div class="user-txt" style="left:{user_x}%; top:{user_y}%;">TÚ</div>
    </div>
    """
    components.html(compass_html, height=520)
